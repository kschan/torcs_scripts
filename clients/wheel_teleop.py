...
def drive(c, inputs):
    ...
    R['steer']= -axis_states['x']  # js gives -1 for full left

    R['accel'] = -(axis_states['y'] - 1)/2.
    R['brake'] = (-(axis_states['z'] - 1)/2.)**2*0.5

    # Traction Control System
    if ((S['wheelSpinVel'][2]+S['wheelSpinVel'][3]) -
       (S['wheelSpinVel'][0]+S['wheelSpinVel'][1]) > 10):
       R['accel']-= .2
    ...
    if button_states['base5']: # left thumb button force reverse
        R['gear'] = -1

    if button_states['top2']:     # right trigger force upshift
        R['gear'] = S['gear'] + 1
    elif button_states['pinkie']:   # left trigger force downshift
        R['gear'] = S['gear'] - 1
...
if __name__ == "__main__":
    ...
    for step in range(C.maxSteps):
        ...
        if (response == -1):
            print('Shutdown at step %d' % step)
            break
        drive(C, inputs)
        if not inputs.button_states['thumb2']: # skip logging if circle button pressed
            logged_states[index, :] = statesAsArray(C.S.d)
            logged_inputs[index, :] = inputsAsArray(C.R.d)
            index += 1
        else:
            print("skipping logging step %d" % step)
        C.respond_to_server()
